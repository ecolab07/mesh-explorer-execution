...
describe("CT-C-* Cursor semantics per principal", () => {
  ...
  it("CT-C-2: alternating visibility across principals has no observable holes", async () => {
    const store = new InMemoryLocalEventStore();
  ...
  it("CT-C-3 contradiction: hidden tx cannot be read directly by masked principal", async () => {
    const store = new InMemoryLocalEventStore();
